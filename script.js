...
function charSpeech() {
  ...
  /* RiTa.sentences = rm.generateSentences(10);

  for (i = 0; i < RiTa.sentences.length; i++) {
    saySentences.push(RiTa.sentences[i]);
 }
  
  RiTa.response = rm.generateSentences(10);

  for (i = 0; i < RiTa.response.length; i++) {
    respondSentences.push(RiTa.response[i]);
 } */
  
  var speech = "It approached the " + prevChar + ", " + verbedAdverb() + " to it, " + "and said: " + " <br/></br/>'Excuse me, but I was wondering: " + "test" + "'" + "<br/><br/>'Actually,' responded the " + prevChar + ` ${RiTa.randomWord('rb')}` + ", 'I think you'll find the following. " + "test" + "' " + "The " + currentChar + " felt " + `${RiTa.randomWord('jj')} ` + "at this, and " + verbedAdverb() + ". <br/>";
